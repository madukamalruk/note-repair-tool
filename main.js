...
module.exports = class NoteRepairToolPlugin extends Plugin {
  ...
  fixTikz(text) {
    ...
    let fixedText = text.replace(tikzRegex, (match, code) => {
      ...
      // Strip documentclass and begin/end document wrappers generated by AI
      let cleaned = code.replace(/\\documentclass(\[[^\]]*\])?\{[^}]*\}/g, '');
      cleaned = cleaned.replace(/\\usepackage\{circuitikz\}/g, '');
